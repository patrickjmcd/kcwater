...
def valid_charge_date(history_obj):
    ...
    if d.date() == today and history_obj['readDateTime']:
        ...
        d_hour = int(time_split[0]) if time_split[1] == "AM" else (
            int(time_split[0]) + 12)
...
class KCWater():
    ...
    def get_token(self):
        ...
        login_payload = {"username": str(self.username), "password": str(
            self.password), "grant_type": "password"}
        r = self.session.post(url=self.tokenUrl, data=login_payload, headers={
    ...
    def get_customer_info(self):
        ...
        r = self.session.post(url=self.customer_info_url, data=json.dumps(
            info_payload), headers=self.headers)
    ...
    def get_usage_hourly(self, date=today):
        ...
        usageData = self.session.post(self.hourly_usage_url, data=json.dumps(
            req_payload), headers=self.headers).json()
    ...
    def get_usage_daily(self, date=yesterday):
        ...
        usageData = self.session.post(self.daily_usage_url, data=json.dumps(
            req_payload), headers=self.headers).json()
        if log_level == logging.DEBUG:
...
if __name__ == "__main__":
    ...
    kc_water.login()
    logging.debug("Account Number = {}, service ID = {}, customer ID = {}".format(
    ...
    logging.info("Last daily reading: {} gal for {}".format(
        daily_data[-1]["gallonsConsumption"], daily_data[-1]["readDate"]))
    logging.info("Last hourly reading: {} gal for {} {}".format(
        hourly_data[-1]["gallonsConsumption"], hourly_data[-1]["readDate"], hourly_data[-1]["readDateTime"]))
